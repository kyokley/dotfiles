...
if [ -n "$XMLSTARLET" ]; then
    TOKEN=$(OPENSSL_CONF=<(echo $OPENSSL_CONFIG) curl $PINNEDPUBKEY -s "$URL"  | xmlstarlet sel -t -v /hostscan/token)
else
    TOKEN=$(OPENSSL_CONF=<(echo $OPENSSL_CONFIG) curl $PINNEDPUBKEY -s "$URL" | sed -n '/<token>/s^.*<token>\(.*\)</token>^\1^p' )
fi

if [ -n "$XMLSTARLET" ]; then
    URL="https://$CSD_HOSTNAME/CACHE/sdesktop/data.xml"

    OPENSSL_CONF=<(echo $OPENSSL_CONFIG) curl $PINNEDPUBKEY -s "$URL" | xmlstarlet sel -t -v '/data/hostscan/field/@value' -n | while read -r ENTRY; do
...
OPENSSL_CONF=<(echo $OPENSSL_CONFIG) curl $PINNEDPUBKEY -s -H "$CONTENT_HEADER" -H "$COOKIE_HEADER" -H 'Expect: ' --data-binary @$RESPONSE "$URL" > $RESULT
