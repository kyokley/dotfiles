{
  flake.modules = {
    ...
    homeManager.hyprland = {
      ...
      # Keep this mutable so hyprmoncfgd can replace it when applying a profile.
      home.activation.ensureHyprmoncfgMonitors = lib.hm.dag.entryAfter ["writeBoundary"] ''
        $DRY_RUN_CMD mkdir -p "$HOME/.config/hypr"
        if [[ ! -e "$HOME/.config/hypr/monitors.lua" ]]; then
          $DRY_RUN_CMD touch "$HOME/.config/hypr/monitors.lua"
        fi
      '';
      ...
      wayland.windowManager.hyprland = {
        ...
        # Load the mutable monitors config managed by hyprmoncfgd.  The Home
        # Manager activation hook above creates an empty placeholder before the
        # first profile is applied.  Hyprland does not add the config directory
        # to package.path, so it has to be done here.
        extraConfig = ''
      ...
      # hyprmoncfgd: monitor profile daemon (auto-apply best-matching profile
      # on hotplug/lid events).  Runs as a user service bound to the Wayland
      # session, mirroring the packaged unit (nixpkgs ships it under
      # share/systemd/user, which systemd does not pick up on its own).
